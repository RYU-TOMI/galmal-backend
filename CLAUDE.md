...
- `main`에는 **크론(github-actions)이 매일 `data/`·`docs/`를 커밋**한다 → push 전 반드시 pull.
  ...
  - 재빌드했으면 **커밋 전에 확인한다** — **딜 수와 제휴 링크 수가 같아야 한다**:
    ...
    > 2026-09-08 **125**건, 09-09 **133**건. 이 문서가 한때 「→ 125」라고 적어둬서
    > 정상인 사람이 133을 보고 고장으로 오인할 상태였다. **불변식은 「둘이 같다」**이지
    > 특정 숫자가 아니다.
    > ⚠️ `grep -c`도 쓰지 말 것. `deals.json`은 **개행이 없는 한 줄**이라 `grep -c`는 매칭된
    > *줄 수*를 세서 **건수와 무관하게 항상 `1`**이다. 전량 손실만 걸리고 **부분 손실은 통과한다**
    > (133건 중 3건만 남아도 `1`).
    > 두 함정 다 2026-09-08~09에 이 문서가 실제로 틀리게 적고 있었다.
