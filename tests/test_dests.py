...
enum 값(region/haul)은 프론트에 약속한 목록이라 `contract/v1/vocab.json`에서 읽는다
(BE10 T3). 예전엔 리터럴로 옮겨 적었는데, 손 사본은 계약이 바뀌어도 조용하다.
사전에 새 값이 생기면 계약 위반으로 잡힌다.

**태그 배정의 정본은 `dests.DEST`다.** 예전엔 기획의 `TAGS.md` 배정표와 순서까지
대조했는데, 저장소가 갈리면서 그 문서가 두 벌이 됐다(R8). 어휘 안에 있는지·하위가
상위를 데리고 다니는지는 `test_contract.py`가 `vocab.json`으로 본다.
"""
import json
import math
import sys
...
_VOCAB = json.loads((Path(__file__).resolve().parent.parent
                     / "contract" / "v1" / "vocab.json").read_text(encoding="utf-8"))
REGIONS = set(_VOCAB["region"])
HAULS = set(_VOCAB["haul"])
