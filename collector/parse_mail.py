...
def main():
    ...
    if failed:
        ...
        # 실패한 메일은 `emails.parsed=0`으로 남아 **다음 실행이 다시 받아 온다**(BB33 — 메일함은 안 건드린다).
        # 그러니 데이터 손실은 없다. 다만 원인(키·한도·장애)을 고치기 전에는 매 실행 같은 메일이 또 실패한다.
        raise SystemExit(
            f"메일 파싱 경보: {len(failed)}/{len(rows)}통이 API 오류로 실패했다. "
            "401 이면 API 키가 무효다(Console 에서 새 키 → production 환경 시크릿 ANTHROPIC_API_KEY 교체). "
            "그 밖엔 월 지출 한도·API 장애를 확인할 것. 실패한 메일은 다음 실행이 자동으로 다시 시도한다.")
